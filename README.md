...
| `lowest_price` | ❌ AI 금지 | 항상 `null` (미구현, `missing_reasons` 기록) |
  ...
  "products": [
    {
      ...
      "partner_product": {
        ...
        "ai_fields": [],
        "missing_reasons": {
          "lowest_price": "시장 최저가 조회 미구현 — Naver 쇼핑 카탈로그 API 연동 필요",
...
- **시장 최저가 (`lowest_price`)**: Naver 쇼핑 카탈로그 API 또는 가격비교 페이지 실크롤링으로 구현 가능하지만 인증 요구 및 빈번한 구조 변경으로 별도 안정화가 필요하다. 오탐 방지(악의적 저가, 단종 상품)와 수집 시각 기록도 함께 구현해야 한다.
