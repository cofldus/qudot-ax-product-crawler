...
# 네이버 쇼핑 최저가 실크롤링 활성화 (속도 느려짐)
python main.py --url https://brand.naver.com/kefii --max-products 5 --lowest-price

# 증분 재크롤 — 24h 이내 수집한 URL 건너뜀 (Supabase 필요)
python main.py --url https://brand.naver.com/kefii --max-products 10 --incremental

# 두 플래그 동시 사용
python main.py --url https://brand.naver.com/kefii --max-products 10 --lowest-price --incremental
```
...
|---|---|---|
| `GET` | `/health` | 헬스 체크 |
| `POST` | `/crawl` | 스토어 크롤링 및 AI 정규화 |
| `GET` | `/products` | 수집된 상품 목록 조회 (Supabase 필요) |
| `GET` | `/review` | 검수 UI — HTML 테이블 시각화 (Supabase 필요) |
...
검수 UI: `http://localhost:8000/review`
...
### `--lowest-price` — 네이버 쇼핑 최저가 실크롤링

`--lowest-price` 플래그 활성화 시 네이버 쇼핑 검색 결과 페이지(`search.shopping.naver.com`)에서 동일 상품의 최저가를 실시간으로 수집한다.

오탐 방지를 위한 3중 검증을 적용한다:

1. `sales_price`가 없으면 수집 건너뜀 (비교 기준 없음)
2. 검색 결과 상품명과 원본 상품명의 Jaccard 유사도 0.35 이상만 신뢰
3. 수집된 최저가가 판매가의 10% 미만이면 이상값으로 폐기

수집 근거는 `raw_evidence.lowest_price`에 기록된다:

```json
"lowest_price": {
  "value": 18900,
  "matched_title": "케피 버블클렌저 200ml",
  "similarity": 0.62,
  "raw_price_text": "18,900원",
  "selector": ".price_num",
  "collected_at": "2026-06-16T10:23:41"
}
```

비활성화 시(기본값) `lowest_price`는 `null`이고 `missing_reasons`에 사유가 기록된다:
...
Supabase `partner_products` 테이블의 `source_url`·`crawled_at`을 조회해 24시간 이내 수집된 URL은 건너뛴다. `summary.skipped_count`로 건너뛴 수를 리포트한다.

```bash
python main.py --url https://brand.naver.com/kefii --max-products 20 --incremental
# [완료] ...
#   발견:    100개 / 건너뜀:  17개 (증분 재크롤)
```

Supabase 미설정 시 `--incremental` 플래그는 무시되고 전체 재크롤된다.

### `GET /review` — 검수 UI

`http://localhost:8000/review`에서 Supabase에 저장된 상품을 HTML 테이블로 시각화한다. 이미지 미리보기·상태 뱃지·가격·출처 링크를 한 화면에서 확인할 수 있다.

모든 문자열 출력에 `html.escape`를 적용해 XSS 입력을 방지한다.

### 요청별 설정 격리

`settings.model_copy(update={...})`로 전역 설정을 변경하지 않고 요청별 독립 복사본을 생성한다. FastAPI 다중 동시 요청에서 `lowest_price`·`incremental` 플래그가 서로 누출되지 않는다.
...
※ `total_discovered` 수치는 실행 시점·캐시 상태에 따라 달라질 수 있다.
...
- **happylandmall consumer_price**: 현재 `sales_price`만 추출됨. 정가 셀렉터 추가로 할인율 계산도 가능하다. 고도몰 템플릿별 셀렉터 다양성이 걸림돌이다.
- **phytonutri 인증 우회 없는 대안**: 로그인 없이 접근 가능한 스마트스토어 공개 상품 API(상품 ID만 있으면 조회되는 경로)가 있다면 해당 경로를 1순위로 시도하는 분기를 추가할 수 있다.
- **병렬 크롤링**: 현재 상품별 순차 수집. `asyncio.gather`로 병렬화하면 처리량을 높일 수 있으나 요청 빈도 제한 처리가 필요하다.
...
- **분산 크롤링**: 대규모 카탈로그(수천 SKU)에는 Celery + Redis 큐로 병렬 수집 워커를 구성해 처리량을 높일 수 있다. 현재는 단일 프로세스 순차 수집이다.
